#include "SampleAnalyzer/User/Analyzer/cms_exo_23_003_SidebandA.h"
using namespace MA5;
using namespace std;
...
// -----------------------------------------------------------------------------
bool cms_exo_23_003_SidebandA::Execute(SampleFormat& sample, const EventFormat& event)
{
  ...
  float const Ak15_NTRACKS_MIN = 21;
  float const AK4_PT_MAX = 135;
  ...
  // Sideband A
  bool LowerAk15NTracks = (Ak15jets.at(0).ntracks() >= Ak15_NTRACKS_MIN);
  bool UpperAk4pT = (Ak4jets.at(0).pt() < AK4_PT_MAX);
  bool sidebandA = (LowerAk15NTracks && UpperAk4pT);
  if (not Manager()->ApplyCut(sidebandA, "sidebandA")) return true;
...
// -----------------------------------------------------------------------------
void cms_exo_23_003_SidebandA::Finalize(const SampleFormat& summary, const std::vector<SampleFormat>& files)
{
}
