#include "SampleAnalyzer/cms_exo_23_003_CRTT/Analyzer/cms_exo_23_003_CRTT.h"
using namespace MA5;
using namespace std;
...
bool BTag(vector<RecJetFormat> jets){
...
// -----------------------------------------------------------------------------
MAbool cms_exo_23_003_CRTT::Initialize(const MA5::Configuration& cfg,
...
// -----------------------------------------------------------------------------
bool cms_exo_23_003_CRTT::Execute(SampleFormat& sample, const EventFormat& event)
{
  ...
  // One Ak15 cluster and one Ak4 cluster
  ...
  // BTag
  bool yesBTag = BTag(Ak4jets);
  if (not Manager()->ApplyCut(yesBTag, "yesBTag")) return true;
  ...
  bool dROverlap = (calculateDeltaR(Ak4jets.at(0), Ak15jets.at(0)) < DR_MAX);
...
// -----------------------------------------------------------------------------
void cms_exo_23_003_CRTT::Finalize(const SampleFormat& summary, const std::vector<SampleFormat>& files)
{
}
